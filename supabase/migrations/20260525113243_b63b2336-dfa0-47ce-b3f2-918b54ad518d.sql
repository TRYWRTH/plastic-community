...
AS $$
  WITH ranked AS (
    SELECT
      es.event_id,
      upper(left(coalesce(u.email, ''), 1)) AS initial,
      es.created_at,
...
REVOKE EXECUTE ON FUNCTION public.get_event_going_initials() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_event_going_initials() TO anon, authenticated;
